...
def get_inputs(args, traj_data, gt_trajs, seg_imgs):
    ...
    # pred_timesteps = [len(gt_trajs[traj_id][future_id]["x_agent_traj"]) for future_id in gt_trajs[traj_id]]
    pred_timesteps = len(gt_trajs[1]["x_agent_traj"])
    pred_length_list.append(pred_timesteps)
    # ---------------------------------------------------------------------
